-- Quotes persistence: store a snapshot of every gateway quote sent to a merchant.
--
-- Until now the QuoteGeneratorDialog produced an ephemeral PDF + email with no
-- record kept. This adds a `quotes` table that captures the full snapshot
-- (tier, billing, line items, totals, client + sender contact info) at send
-- time so we can audit, re-render, expire, and track acceptance later.
--
-- Linkage is loose by design: a quote may be tied to an opportunity / account /
-- contact, but doesn't have to be (e.g. one-off quotes from /tools/quote-builder
-- for a prospect that doesn't exist in the CRM yet). FKs are ON DELETE SET NULL
-- so deleting a CRM record never wipes the audit trail.
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Row Level Security — internal staff only.
-- Referrers (affiliate portal users) are excluded via public.is_referrer().
