-- Quote acceptance: enables the public Accept-&-Sign flow.
--
-- The QuoteGeneratorDialog now embeds a tokenized URL (https://app/q/:token)
-- in the merchant's quote email. Clicking that link opens a public acceptance
-- page where the merchant signs (name/title/email + authority-to-bind + MSA
-- agree, with optional ACH details). On submit, the `accept-quote` edge
-- function validates the token, writes an immutable audit row to
-- `quote_acceptances`, flips quotes.status to 'accepted', and notifies the
-- CRM owner so they can generate the contract.
...
CREATE TABLE IF NOT EXISTS public.quote_acceptances (
  ...
  -- Optional ACH authority captured at accept (last 4 only — never store full account)
...
-- ─── 3. RLS policies ───────────────────────────────────────────────────────
-- Internal staff can read all acceptances. The accept-quote edge function
-- uses service role and bypasses RLS to insert.
...
-- No UPDATE/DELETE policy — acceptances are append-only audit records.
-- Service role can still mutate for ops emergencies; that bypass is logged at the platform level.

-- ─── 4. View — quote_acceptance_summary ────────────────────────────────────
-- Convenience view for the CRM UI ("Quote X accepted by Y on Z").
