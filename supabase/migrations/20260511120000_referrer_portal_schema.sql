...
--
-- Adds an external "referrer" user concept distinct from internal CRM staff:
--   * Each referrer has a Supabase auth user (admin-provisioned, password-based).
--   * Referrers own opportunities/accounts/applications they referred to us.
--   * Commissions: flat % of monthly net revenue per referred merchant, capped per merchant per month.
--
-- Internal CRM staff are unaffected — existing RLS still grants them full access via
-- `auth.uid() IS NOT NULL` policies. These additions only grant referrers a *narrower* slice.
...
CREATE TABLE IF NOT EXISTS public.referrers (
  ...
  monthly_cap_per_merchant    numeric(10,2) NOT NULL DEFAULT 1500.00,  -- USD cap per merchant per month
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Helper functions (SECURITY DEFINER so they bypass RLS during evaluation)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Add referrer_id FK columns to existing tables
-- ─────────────────────────────────────────────────────────────────────────────
...
-- merchants table may exist in either legacy or new form; ADD COLUMN IF NOT EXISTS is safe.
DO $$
BEGIN
...
CREATE POLICY "Referrers can view their own profile"
  ...
  USING (auth_user_id = auth.uid() OR public.is_admin_email() OR (
    -- Internal staff (any authenticated user with @merchanthaus.io domain) can also read
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Extend RLS on opportunities/accounts/applications/merchants so referrers
-- can SELECT only their own rows, in addition to existing internal-staff policies.
-- ─────────────────────────────────────────────────────────────────────────────

-- Opportunities: referrers can see their own
...
-- Accounts: referrers can see their own
...
-- Applications: referrers can view their own + insert new ones tagged with their id
...
-- Merchants: referrers see their own (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'merchants') THEN
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Propagate referrer_id from applications → opportunities when converted.
-- (The conversion flow in /admin/web-submissions creates an opportunity from an
--  application. We tag the resulting opportunity if any nearby application has
--  a matching email + referrer_id within the past 90 days.)
--
-- Lightweight: just expose a SQL function the conversion code can call, rather
-- than a trigger (we don't have a direct FK from opportunities → applications).
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF v_referrer_id IS NOT NULL THEN
    ...
    -- Also tag the account if the opportunity has one
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Commission visibility for referrers.
--
-- The internal `commission_records` table holds what the COMPANY earned per
-- merchant per period. Referrer payout = MIN(total_commission * rate, cap).
--
-- Existing SELECT policy on commission_records is `auth.uid() IS NOT NULL`,
-- which would expose every referrer's earnings to every other referrer. We
-- tighten it: internal staff still see everything; referrers see only the
-- records tied to accounts they referred.
-- ─────────────────────────────────────────────────────────────────────────────
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'commission_records') THEN
    ...
    EXECUTE $POL$
      CREATE POLICY "Internal staff can view all commission records"
        ...
        USING (
          -- Internal staff: anyone whose email is on the merchanthaus.io domain
          auth.uid() IS NOT NULL AND EXISTS (
            SELECT 1 FROM auth.users
             WHERE id = auth.uid()
...
-- View that computes the referrer's payout per record, with cap applied.
-- Filters automatically via the underlying RLS on commission_records.
...
SELECT
  ...
  -- Capped: min(uncapped, cap) — when cap = 0 we treat as no cap
  CASE
...
COMMENT ON VIEW public.referrer_commission_records IS 'Per-merchant per-period referrer payout with monthly cap applied. RLS on commission_records & accounts scopes referrers to their own rows.';

COMMENT ON TABLE public.referrers IS 'External referral partners with read-only portal access to their own opportunities/accounts and commission visibility.';
COMMENT ON COLUMN public.referrers.commission_rate IS 'Decimal share of monthly net revenue per referred merchant. 0.10 = 10%.';
COMMENT ON COLUMN public.referrers.monthly_cap_per_merchant IS 'USD cap on commission earned per merchant per month. 0 = no cap.';
