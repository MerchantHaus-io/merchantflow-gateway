      ...
      <div className="p-4 md:p-8">
            <div className="max-w-6xl mx-auto space-y-6">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                ...
                <div className="rounded-2xl border border-border bg-card px-4 py-3 shadow-xl w-full md:w-[360px] space-y-2">
                  <div className="flex items-center justify-between text-sm text-foreground">
                    <span>Attach to dashboard account</span>
                    {isLoadingState && <span className="text-xs text-muted-foreground">Loading…</span>}
                  </div>
                  <select
...
function Input(props: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={cn(
        "w-full rounded-lg border border-border bg-muted px-3 py-2 text-sm text-foreground shadow-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20",
        props.className
      )}
    />
  );
...
function OutstandingSummary({ progress, missingBySection }: { progress: number; missingBySection: Record<string, string[]>; }) {
  return (
    <div className="mt-4 rounded-2xl border border-border bg-muted p-4 text-sm">
      ...
      {progress === 100 ? (
        <p className="text-emerald-500">All required items are captured. 🎉</p>
      ) : (
        <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
          {Object.entries(missingBySection).map(([section, fields]) =>
            fields.length ? (
              <li key={section}>
                <span className="font-medium capitalize text-foreground">{section.replace(/_/g, " ")}:</span> {fields.join(", ")}
              </li>
            ) : null
          )}
        </ul>
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-border bg-muted p-3 text-xs space-y-2">
            ...
            {existingDocs.map((doc) => (
              <li key={doc.id} className="flex items-center justify-between py-2 gap-2">
                <div className="min-w-0 flex-1">
                  <p className="text-foreground truncate">{doc.file_name}</p>
                  <p className="text-[10px] text-muted-foreground">
                    {doc.document_type || 'Unassigned'} • {doc.file_size ? `${Math.round(doc.file_size / 1024)} KB` : '—'}
                  </p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <button type="button" onClick={() => handleDownload(doc)} className="rounded p-1 hover:bg-secondary/40 text-muted-foreground hover:text-foreground" title="Download">
                    <Download className="w-3.5 h-3.5" />
                  </button>
                  <button type="button" onClick={() => handleDelete(doc)} className="rounded p-1 hover:bg-destructive/20 text-muted-foreground hover:text-destructive" title="Delete">
        ...
        <div className="rounded-xl border border-dashed border-border bg-muted/50 p-4 text-center text-xs text-muted-foreground">
      ...
      {opportunityId ? (
        <div className="space-y-3 rounded-xl border border-border bg-muted/50 p-4">
          <div className="font-medium text-foreground text-xs">Upload new documents</div>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label="Document type">
              <select
                value={selectedDocType}
                onChange={e => setSelectedDocType(e.target.value)}
                className="w-full rounded-lg border border-border bg-muted px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
              >
            ...
            <Field label="Select files" required>
              <input
                ...
                className="w-full cursor-pointer rounded-lg border border-dashed border-border bg-muted px-3 py-2 text-xs text-muted-foreground"
              />
            </Field>
          </div>
          ...
          <button
            ...
            className="rounded-xl bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground shadow hover:bg-primary/90 disabled:opacity-50"
          >
      ...
      <Field label="Internal notes">
        <textarea
          className="min-h-[90px] w-full rounded-lg border border-border bg-muted px-3 py-2 text-sm text-foreground shadow-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
          value={form.notes}
...
function SummaryItem({ label, value }: { label: string; value: string; }) {
  return (
    <div className="flex justify-between gap-3">
      ...
      <dd className="flex-1 text-right text-foreground">{value || "—"}</dd>
    </div>
  );
}
